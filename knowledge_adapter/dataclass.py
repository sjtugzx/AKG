...
SEP, EOS, CONTEXT_START, CONTENT_START, MASK = '<sep>', '<eos>', '<context_start>', \
                                               '<content_start>', '<MASK>'

# train_masked_knowledge_path, train_knowledge_path
# dev_masked_knowledge_path, dev_knowledge_path

class FillMaskData:
    def __init__(self, train_masked_knowledge_path, train_knowledge_path,
                 test_masked_knowledge_path, test_knowledge_path, max_knowledge_len, max_tgt_len,
                 model_name, train_data_num):

        self.max_knowledge_len, self.max_tgt_len = \
            max_knowledge_len, max_tgt_len
        self.special_token_list = [SEP, EOS, CONTEXT_START, CONTENT_START, MASK]
        ...
        train_src_id_list, train_tgt_id_list, train_src_text_list, train_tgt_text_list = \
            self.load_data(train_masked_knowledge_path, train_knowledge_path)

        self.train_data_num = int(train_data_num * len(train_src_id_list))
        print(print("the entire training data size is: "+ str(len(train_src_id_list))), "the training data size is: "+ str(self.train_data_num))

        self.train_src_id_list = train_src_id_list[:self.train_data_num]
        self.train_tgt_id_list = train_tgt_id_list[:self.train_data_num]
        self.train_src_text_list = train_src_text_list[:self.train_data_num]
        self.train_tgt_text_list = train_tgt_text_list[:self.train_data_num]

        print('Start loading test data...')
        dev_src_id_list, dev_tgt_id_list, dev_src_text_list, dev_tgt_text_list = \
            self.load_data(test_masked_knowledge_path, test_knowledge_path)

        print(print("the entire dev data size is: "+ str(len(dev_src_id_list))))

        self.dev_src_id_list = dev_src_id_list
        self.dev_tgt_id_list = dev_tgt_id_list
        self.dev_src_text_list = dev_src_text_list
        self.dev_tgt_text_list = dev_tgt_text_list
    ...
    def load_text_path(self, path):
        text_list = []
        with open(path, 'r', encoding='utf8') as i:
            lines = i.readlines()
            for l in lines:
                one_text_ = l.strip('\n').split('	')
                for one_text in one_text_:
                    text_list.append(one_text)
                # print("one text: ", one_text)
    ...
    def load_data(self, src_path, tgt_path):
        masked_knowledge_list = self.load_text_path(src_path)
        knowledge_list = self.load_text_path(tgt_path)
        src_id_list = self.load_text_id_list(masked_knowledge_list, self.max_knowledge_len)
        tgt_id_list = self.load_text_id_list(knowledge_list, self.max_knowledge_len)
        tgt_id_list = [[self.bos_token_id] + item + [self.eos_token_id] for item in
                            tgt_id_list]
        return src_id_list, tgt_id_list, masked_knowledge_list, knowledge_list
    ...
    def get_next_train_batch(self, batch_size):
        ...
        # source-side input
        batch_src_tensor = rnn.pad_sequence(batch_src_id_list, batch_first=True,
        ...
        # target-side input
        batch_tgt_tensor = rnn.pad_sequence(batch_tgt_id_list, batch_first=True,
                                            padding_value=self.pad_token_id)
        if self.model_name.startswith('t5'):
            batch_tgt_in_tensor, batch_tgt_out_tensor = self.process_t5_decoder_tensor(
                batch_tgt_tensor)
        else:
            batch_tgt_in_tensor, batch_tgt_out_tensor = self.process_decoder_tensor(
                batch_tgt_tensor)
...
class Data:
    def __init__(self, train_table_path, train_summary_path, train_context_path, train_content_path, train_data_num, test_table_path, test_summary_path,
                 test_context_path, test_content_path, max_table_len, max_tgt_len, max_context_len,
                 context_num, max_content_len, model_name, add_context=True, add_content=True,
                 prefix_tuning=False):
    ...
    def load_one_text_prefix_id(self, text, max_len):
        text_id_list = self.tokenizer.encode(text, max_length=150, truncation=True,
    ...
    def load_content_id_list(self, content_text_list):
        ...
        for text in content_text_list:
            ...
            if self.prefix_tuning:
                one_res_id_list = self.load_one_text_prefix_id(one_res_text, self.max_content_len)
            else:
