...
if __name__ == '__main__':
    ...
    if args.dataset_name == 'human':
        remove_slot_key_list = ['caption', 'death date', 'name', 'article title',
                                'image', 'fullname', 'full name', 'birthname', 'birth name',
                                'alias',
                                'othername', 'imdb', '|name', '|imagesize', 'othername',
                                'image caption', 'image size']
    elif args.dataset_name == 'book':
        remove_slot_key_list = ['name', 'author', 'publisher', 'publication date', 'written by',
                                'country']
    else:  # song
    ...
    data = Data(args.model_name, train_table_path, train_summary_path, train_context_path,
                ...
                args.train_candi_pool_size, args.train_negative_num, args.test_candi_span,
                args.max_table_len,
            ...
            with torch.no_grad():
                ...
                for test_step in range(test_step_num):
                    ...
                    test_batch_summary_text_list, test_batch_candidate_summary_list = data.get_next_test_batch(
                        batch_size)
                    test_batch_score = trainer(model, test_all_batch_token_list,
                                               test_all_batch_mask_list,
                                               test_all_batch_seg_list, cuda_available, device)
                    test_batch_select_text = extract_test_pred_text(test_batch_score,
                                                                    test_batch_candidate_summary_list)
